...
- Хранит пользователей и OAuth2‑сущности в **MongoDB**.
...
1) Запустить MongoDB (любым способом):
```bash
docker run -d --name sas_mongo -p 27017:27017 mongo:7
```
...
3) Запуск через Java:
```bash
MONGO_URI=mongodb://localhost:27017/gera \
...
docker run --rm -p 9000:9000 --name auth_server \
  -e MONGO_URI=mongodb://host.docker.internal:27017/gera \
...
- `SecurityConfig` — настройка SAS, OIDC, CORS; доступ открыт к `/api/auth/register`, Swagger UI.
- `ClientInitializer` — регистрирует SPA‑клиент (PKCE) и служебных клиентов для тестов.
- Mongo collections: `users`, `oauth2_registered_client`, `oauth2_authorization`, `oauth2_authorization_consent`.
...
- Настройте Mongo auth/ACL и мониторинг индексов на коллекциях авторизации.
